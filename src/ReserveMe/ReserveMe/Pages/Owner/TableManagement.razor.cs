...
public partial class TableManagement : ComponentBase
{
    // Filters
...
private void OnTimeChanged(ChangeEventArgs e)
{
    ...
    private void SaveTable()
    {
        if (_isEditMode)
        {
            var existing = _tables.FirstOrDefault(t => t.Id == _editingTable.Id);
            if (existing is not null)
            {
                existing.TableNumber = _editingTable.TableNumber;
                existing.Capacity = _editingTable.Capacity;
                existing.Status = _editingTable.Status;
                existing.IsActive = _editingTable.IsActive;

                if (_selectedTable?.Id == existing.Id)
                {
                    _selectedTable = existing;
                }
            }
        }
        else
    ...
    {
        if (_selectedTable == null || _selectedTable.Status != TableStatus.Available) return;
    ...
    private void ChangeTableStatus(TableViewModel table, TableStatus newStatus)
    {
    ...
    // Mock Data
    private static List<TableViewModel> GetMockTables() => new()
    {
        new() { Id = Guid.NewGuid(), TableNumber = 1, Capacity = 2, Status = TableStatus.Available, IsActive = true },
        new() { Id = Guid.NewGuid(), TableNumber = 2, Capacity = 4, Status = TableStatus.Occupied, IsActive = true, ActiveReservationId = Guid.NewGuid() },
        ...
        new() { Id = Guid.NewGuid(), TableNumber = 6, Capacity = 8, Status = TableStatus.Occupied, IsActive = true, ActiveReservationId = Guid.NewGuid() },
        new() { Id = Guid.NewGuid(), TableNumber = 7, Capacity = 4, Status = TableStatus.Available, IsActive = false },
        new() { Id = Guid.NewGuid(), TableNumber = 8, Capacity = 4, Status = TableStatus.Available, IsActive = true },
        new()
        {
            Id = Guid.NewGuid(),
            TableNumber = 9,
        ...
        new() { Id = Guid.NewGuid(), TableNumber = 10, Capacity = 10, Status = TableStatus.Available, IsActive = true },
    };
}
